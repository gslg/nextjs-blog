...
export function getSortedPostsData(): number {
  // Sort posts by date
...
function parseMD(filepath: string, fileName: string) {
...
function replaceImages(str) {
  ...
  return str.replace(imgReg, function (match, p1, p2, p3, p4) {
    ...
    console.log("p3=" + p3);
    console.log("p4=" + p4);
    //let a = "/" + path.relative(publicDirectory, p3) 在其他机器上部署就不行了
    const a = p3.substring(p3.lastIndexOf("/images"));
    console.log("图片地址:" + a);
